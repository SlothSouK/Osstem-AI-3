...
7. **TOTVS 스크래퍼** (`totvs/`) — TOTVS ERP 웹 로그인 → 데이터 수집 (Playwright)
...
│   ├── app.py            Streamlit 웹 앱 (해외관리실의 공간)
...
│   └── requirements.txt
├── totvs/                TOTVS ERP 웹 스크래퍼 (Playwright)
│   ├── scraper.py        실행 파일 (로그인 → 데이터 수집)
│   ├── config/
│   │   ├── config.ini    headless, timeout 등 스크래퍼 옵션
│   │   └── .env          TOTVS_URL, TOTVS_USER_ID, TOTVS_PASSWORD (git 제외)
│   ├── src/              (확장용 모듈 디렉토리)
│   └── requirements.txt
└── CLAUDE.md
```
...
- **검증**: 각 계정마다 사전(조회 시작일 -1일) + 사후(조회 종료일) 두 차례 실행
  - FBL5N 미결항목 모드 → **주요일자의 미결항목** 필드(`ctxtPA_STIDA`)에 기준일 입력 (YYYY.MM.DD)
  - 일반항목(외화외상매출금(잔액)) + 특별G/L(미수금(잔액)) 각각 별도 조회
  - SAP 합계: 소계 행 기준 (고객 컬럼에 값, 기호·지정·유형·D/C·G/L 빈칸인 행) / 조회 결과 없으면 0 처리
  - 엑셀 합계: 금액 헤더 열 헤더 영역의 수식 결과 셀 기준
  - 결과를 텍스트 파일에 기록하며 일치 여부 판단
...
- 브라우저에서 조건 입력 → 확인 → SAP 실행 → 실시간 로그 표시
- 사이드바에서 메뉴 선택 (현재: 채권명세서 / 추후 모듈 추가 가능)
- `fbl5n_download.py` 핵심 함수 직접 재사용, 별도 프로세스 불필요
...
TOTVS ERP 웹에 Playwright로 로그인하여 데이터를 수집한다.

```bash
# 의존성 설치
pip install -r totvs/requirements.txt
playwright install chromium

# 실행
python totvs/scraper.py
```

실행 전 필수 설정:
- `totvs/config/.env` — TOTVS_URL, TOTVS_USER_ID, TOTVS_PASSWORD
- `totvs/config/config.ini` — headless(true/false), timeout(ms)
...
| TOTVS 스크래핑 | Playwright 1.44, python-dotenv 1.0 |
